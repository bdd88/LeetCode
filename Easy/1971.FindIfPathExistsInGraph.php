...
class Solution {
    function validPath($n, $edges, $source, $destination) {
        ...
        $visited = array();
        $this->dfs($map, $visited, $source, $destination);
        return isset($visited[$destination]);
    }

    function dfs($map, &$visited, $source, $destination) {
        $visited[$source] = TRUE;
        if ($source === $destination) return;
        foreach ($map[$source] as $neighbor) {
            if (!isset($visited[$neighbor])) {
                $this->dfs($map, $visited, $neighbor, $destination);
            }
        }
    }
}
...
class Solution {
    ...
    function union($first, $second) {
        $firstRoot = $this->find($first);
        $firstHeight = $this->height[$firstRoot];
        $secondRoot = $this->find($second);
        $secondHeight = $this->height[$secondRoot];
        if ($firstRoot !== $secondRoot) {
            if ($firstHeight > $secondHeight) {
                $this->root[$secondRoot] = $firstRoot;
            } elseif ($firstHeight < $secondHeight) {
                $this->root[$firstRoot] = $secondRoot;
            } else {
                $this->root[$secondRoot] = $firstRoot;
                $this->height[$firstRoot]++;
            }
        }
    }
